--[[
    Shield Core Bubble
    Copyright (C) 2011 Madman07 (shield core now works for gmod 13 completely)
]]

if not StarGate or not StarGate.CheckModule or not StarGate.CheckModule("devices") then return end
...
StarGate.Trace:Add("shield_core_bubble",
        ...
        if not e.Depleted and e.Enabled then
            ...
            if type(values[3]) == "table" then
                ...
                if IsValid(values[3][1]) then
                    values[3][1]:SetNetworkedEntity("SC_Owner", own)
                end
            else
...
function ENT:PhysicsCollide(data, physobj)
    ...
    timer.Create("Delay"..e:EntIndex(), 0.1, 0, function()
        if IsValid(self) then
            self.HitDelay[e] = false
        end
    end)
...
local function CalcDmgProtect(ent, inflictor, attacker, amount, dmginfo)
    if IsValid(ent) and ent:IsPlayer() then
        if IsValid(inflictor) then
            ...
            local start = inflictor:LocalToWorld(inflictor:OBBCenter()) - inflictor:GetVelocity():GetNormal() * 20
